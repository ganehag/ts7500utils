/*
 * Copyright (c) 2006-2011, Technologic Systems
 * All rights reserved.
 */
...
static int error(unsigned int *resp, unsigned short req) {
  ...
  unsigned int iovcnt) {
	...
	if (sd->sd_erasehint) {
		...
		ret = sdcmd2(sd, ACMD_SET_WR_BLK_ERASE_COUNT, sd->sd_erasehint, 
		  resp, NULL);
		if (ret || error(resp, ACMD_SET_WR_BLK_ERASE_COUNT)) return 1;
		sd->sd_erasehint = 0;
	}

	if (sd->sd_nomultiwrite) for (;;) {
		if (sd->sd_state & SD_HC)
		  ret = sdcmd2(sd, CMD_WRITE_BLOCK, sector, resp, NULL);
		else
		  ret = sdcmd2(sd, CMD_WRITE_BLOCK, sector * 512, resp, NULL);

		if (ret || error(resp, CMD_WRITE_BLOCK)) return 1;
...
int sdreset(struct sdcore *sd) {
	...
	do {
		...
		if (i > 1000) sd->sd_timeout = 1000001;
		if (timeout(sd)) break;
		i++;
	} while (((resp[1] & 0x80) == 0x0));
